            ...
            let action = UIAlertAction(title: artist.name, style: .default) { _ in
                if let index = self.artists.firstIndex(where: { $0.id == artist.id }) {
                    if SharedData.shared.albums.contains(where: { $0.artistId == artist.id }) {
                        self.showAlert(title: "Error", message: "Cannot Delete Customer With Associated Albums.")
                    } else {
        ...
        for artist in artists {
            let action = UIAlertAction(title: artist.name, style: .default) { _ in
                ...
                let updateAction = UIAlertAction(title: "Update", style: .default) { _ in
                    if let newName = alert.textFields?.first?.text, !newName.isEmpty, let index = self.artists.firstIndex(where: { $0.id == artist.id }) {
